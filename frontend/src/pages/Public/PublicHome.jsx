...
function DetailsModal({ detail, onClose }) {
  /* -------------------------------------------------------
     SCROLL LOCK (JS fallback)
     The CSS also has `body:has(.public-detail-backdrop)`,
     but :has() isn't supported on every phone/browser.
     This effect guarantees the background never scrolls
     behind the modal, on any device, regardless of :has()
     support. It also lets Escape close the modal.
  ------------------------------------------------------- */
  useEffect(() => {
...
function PublicHome({
  ...
  /* -------------------------------------------------------
     DETAIL MODAL BROWSER HISTORY
     Opening a detail modal creates a history entry.
     Browser Back closes the modal instead of leaving
  ...
  const closeDetail = useCallback(() => {
    if (window.history.state?.publicDetailModal) {
      window.history.back();
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!window.history.state?.publicHomeGuard) {
      window.history.pushState(
        {
          ...(window.history.state || {}),
          publicHomeGuard: true,
        },
        "",
        currentUrl
      );
    ...
    const handlePopState = () => {
      // Ignore the popstate generated by history.forward()
      if (isRestoringHistory.current) {
        isRestoringHistory.current = false;
        return;
      }

      // Back from modal: close only the modal
      if (selectedDetailRef.current) {
        setSelectedDetail(null);
        return;
      }

      // Back from the first public page: remain on public page
                  ...
                  <div>
                    ...
                    <div className="public-card-actions">
                      <button
                        type="button"
                        className="public-view-details-button"
                        onClick={() => openDetail("experience", item)}
                      >
                ...
                >
                  ...
                  <div className="public-card-actions">
                    <button
                      type="button"
                      className="public-view-details-button"
                      onClick={() => openDetail("education", item)}
                    >
                ...
                >
                  ...
                  <div className="public-card-actions">
                    <button
                      type="button"
                      className="public-view-details-button"
                      onClick={() => openDetail("certification", item)}
                    >
